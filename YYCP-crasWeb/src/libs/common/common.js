...
class Storage {
    set(key,value,timeout){
        localStorage.setItem(key,JSON.stringify({
            value:value,
            timeout:timeout=='undefined'?undefined:Date.now()+1000*timeout
        }))
    }
    get(key){
        let res = JSON.parse(localStorage.getItem(key))
        if (res) {
            if (res.timeout-Date.now()<0) {
...
export function tableToLine(arr){
	arr.forEach(function(val){
        var brr=Object.keys(val);
		brr.forEach(function(n){
			if(val[n]==null || val[n]=='null'){
				val[n]='--';
			}
		})
	})
