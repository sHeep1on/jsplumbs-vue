...
(function (Q, $) {
    ...
    tooltip.innerHTML = '<ul class="yjcz">\
           <li class="addNode"><p>添加</p></li >\
            <li class="scxl">\
                <p>删除<i></i></p>\
                <div>\
                   <ul>\
            ...
            <li class="suodin jzsy"><p>锁定</p></li>\
            <li class="jiesuo jzsy"><p>解锁</p></li>\
            <li class="zkqb jzsy"><p>全部展开</p></li>\
            <li class="qbss jzsy"><p>全部收缩</p></li>\
            <li class="bjxl">\
            <p>布局<i></i></p>\
            <div>\
                <ul>\
            ...
            <li class="edgemxfx"><p>模型分析</p></li>\
            <li class="bzxx jzsy"><p>备注</p></li>\
  	</ul >'
    // tooltip.className = "tooltip";
    // document.body.appendChild(tooltip);


    $(".bjxl,.scxl").hover(function () {
        var cckd = event.pageX + 210 - $(document.body).width();
        if (cckd > 0) {
            $(this).find('div').css("right", "102px")
        }
        $(this).find('ul').fadeIn()
    }, function () {
        $(this).find('div').css("right", "-101px")
        $(this).find('ul').hide()
    });
...
!function (Q) {
    ...
    ; (function (Q, $) {
        var template = `<div class="graph-export-panel modal fade"><div class="modal-dialog">
                        <div class="modal-content">
                        ...
                        <div>
                        <label>缩放比例: <input class ="graph-export-panel__export_scale" type="range" value="1" step="0.2" min="0.2" max="3">
                        <span class ="graph-export-panel__export_scale_label">1</span></label>
                        </div>
                        <div>
        ...
        function ResizeBox(parent, onBoundsChange) {
            ...
            this.boundsDiv.style.border = "solid 1px #888";
        ...
        function ExportPanel() {
            ...
            var exportImage = function (print, evt) {
                ...
                if (!imageInfo || !imageInfo.data) {
                    ...
                    let blob =base64ToBlob(imageInfo.data); //new Blob([content]);
                    aLink.download =  '关系研判图';
                    aLink.style.display = 'none'
                    aLink.href = URL.createObjectURL(blob);
                    document.body.appendChild(aLink)
                    aLink.click()
                    $('.graph-export-panel').fadeOut("slow");
...
!function (Q, window, document) {
    ...
    Q.Node.prototype.toJSON = function (serializer) {
        ...
        exportElementProperties(serializer, ['suspicion','_reftype','_refId','location', 'fileOrCaseId','colornew', 'suo', 'size', "flag",'idcard', 'nodeType', 'nodeXm', 'image', 'oldImage','matchType','rotate', 'anchorPosition', 'parentChildrenDirection', 'layoutType', 'hGap', 'vGap'], info, this);
        return info;
    };
    ...
    Q.Edge.prototype.toJSON = function (serializer) {
        ...
        exportElementProperties(serializer, ['fromVId','toVId','newWidth','idArr','absId','intimacy','angle', "relation", 'type', 'infoCount', 'colornew', "tooltip", 'from', 'to', "glrsfzh", "fatherSfzh", "parentSfzh", "glr", 'parentXm', "fatherXm", "relPro", 'edgeType', 'angle', 'bundleEnabled', 'pathSegments'], info, this);
        return info;
    };
    ...
    // 点击更新的json渲染页面的方法  可以var json=自创的json
    ...
    PopupMenu.prototype = {
        ...
        renderItem: function (menuItem, zIndex) {
            ...
            if (menuItem.selected) {
                ...
                iVBORw0KGgoAAAANSUhEUgAAABAAAAAQCAYAAAAf8/9hAAAAPklEQVQ4y2P4//8/AyWYYdQA7AYAAZuamlo7ED+
                H4naQGNEGQDX/R8PtpBjwHIsBz+lqAGVeoDgQR1MiaRgAnxW7Q0QEK0cAAAAASUVORK5CYII=')
                 `
            }
            var a = document.createElement("a");
        ...
        getMenuItems: function (graph, data, evt) {
            ...
            if (data) {
                ...
                if (isNode) {
                    var ccgd = event.pageY + $(".tooltip").height() - $(document.body).height();
                    var cckd = event.pageX + $(".tooltip").width() - $(document.body).width();
                    $(".addNode").addClass("jzsy");
                    if (ccgd < 0) {
                        ccgd = 0
                    }
                    if (cckd < 0) {
                    ...
                    $(".tooltip").css({
                        "left": event.pageX - cckd,
                        "top": event.pageY - ccgd,
                        "opacity": 1
                    }).show()
                    document.oncontextmenu = function () {
                        return false//阻止游览器的右击事件
                    }
                    return false//阻止该框架的右击事件

                }

                items.push({
                    text: getI18NString('改名'), action: function (evt, item) {
                ...
                if (isEdge) {
                    ...
                    items.push({
                        text: isDashLine ? getI18NString('实线样式') : getI18NString('虚线样式'), action: function (evt, item) {
                            data.setStyle(Q.Styles.EDGE_LINE_DASH, isDashLine ? null : [3, 5]);
                        }
                    });
                    items.push({
                        text: getI18NString('线条宽度'), action: function (evt, item) {
                    ...
                    items.push({
                        text: getI18NString('线的颜色'), action: function (evt, item) {
                ...
                items.push({
                    text: getI18NString('顶层显示'), action: function (evt, item) {
                        data.zIndex = 1;
                ...
                items.push({
                    text: getI18NString('底层显示'), action: function (evt, item) {
                        data.zIndex = -1;
                ...
                items.push({
                    text: getI18NString('恢复默认层'), action: function (evt, item) {
                        data.zIndex = 0;
                        graph.invalidate();
                    }
                });
                ...
                document.querySelector('.Q-CanvasPanel').oncontextmenu = function () {
                    return false//阻止游览器的右击事件
                }
            }

            //   items.push({
            //     text: getI18NString('清除图层'), action: function () {
            //       graph.clear();
            //     }
            //   })
            //   items.push(Q.PopupMenu.Separator);
            ...
            //   items.push({
            //     text: getI18NString('放大一级'), action: function (evt, item) {
            //       var localXY = graph.globalToLocal(evt);
            //       graph.zoomIn(localXY.x, localXY.y, true);
            //     }
            //   });
            //   items.push({
            //     text: getI18NString('缩小一级'), action: function (evt, item) {
            //       var localXY = graph.globalToLocal(evt);
            ...
            //   var interactons = [
            //     {text: getI18NString('平移模式'), value: Q.Consts.INTERACTION_MODE_DEFAULT},
            //     {text: getI18NString('框选模式'), value: Q.Consts.INTERACTION_MODE_SELECTION}
            //   ];
...
!(function (Q, $) {
    ...
    function createGraphButton(info, scope) {
        ...
        if (info.type == "search") {
            ...
            div.style.top= "11px";
            div.innerHTML = '<div class="input-group input-group-sm canvas-search-ct" >\
            <input type="text" class="form-control" placeholder="' + (info.placeholder || '') + '">\
                <span class="input-group-btn">\
                    <div class="btn btn-default kssearchBtn" ></div>\
                </span>\
            </div>';
        ...
        let exactClass = info.id?(info.id+" "):""
        label.className += exactClass + "btn btn-default btn-sm";
        if (info.icon) {
            var icon = document.createElement('img');
            icon.src = info.icon;
            label.appendChild(icon);
            ...
            var icon = document.createElement('div');
            var iconword = document.createElement('span');
            iconword.innerHTML = info.name;
            $(icon).addClass(info.iconClass);
            label.appendChild(icon);
            label.appendChild(iconword);
        } else if (info.name) {
            label.appendChild(document.createTextNode(" " + info.name));
        }
        if (info.name) {
    ...
    function createToolbar(graph, toolbar, customButtons) {
        ...
        var buttons = {
            ...
            editor4: [
                {
                    name: getI18NString('新建'),
                    iconClass: 'q-icon toolbar-linexj',
                    id:'toolbar-new-graph'
                },
                {
                    name: getI18NString('添加'),
                    iconClass: 'q-icon toolbar-line11',
                    id:'toolbar-add-node'
                },
                {
                    name: getI18NString('保存'),
                    iconClass: 'q-icon toolbar-line8',
                    id:'toolbar-save-graph'
                },
                {
                    name: getI18NString('导出'), iconClass: 'q-icon toolbar-print', action: function () {
            ...
            zoom: [
                ...
                // {
                //     name: getI18NString('放大'), iconClass: 'q-icon toolbar-zoomin', action: function () {
                //         getGraph().zoomIn()
                //     }
                // },
                // {
                //     name: getI18NString('缩小'), iconClass: 'q-icon toolbar-zoomout', action: function () {
                //         getGraph().zoomOut()
                //     }
                // },
                {
                    name: getI18NString('撤销'),
                    iconClass: 'q-icon toolbar-cxcx',
                    id:'toolbar-undo'
                },
                {
                    name: getI18NString('重做'),
                    iconClass: 'q-icon toolbar-cz',
                    id:'toolbar-redo'
                },
                /*      {
                         name: '还原', iconClass: 'q-icon toolbar-zoomreset', action: function () {
                         getGraph().moveToCenter(1,1);// = 1;
                     }
                     }, */
                // {
                //       name: getI18NString('聚焦'),
                //       iconClass: 'q-icon toolbar-zoom_overview',
                //       action: function () {
                //           getGraph().zoomToOverview(1)
                //       }
                //   }
            ],
            editor: [
                {
                    name: getI18NString('连线'),
                    interactionMode: Q.Consts.INTERACTION_MODE_CREATE_EDGE,
                    iconClass: 'q-icon toolbar-edge'
                },
                /*  {
                     name: getI18NString('L线'),
            ...
            editor2: [
                {
                    name: getI18NString('布局'),
                    iconClass: 'q-icon toolbar-line bjtype',
                    id:'toolbar-re-layout'
                },
                {
                    name: getI18NString('锁定'),
                    iconClass: 'q-icon suojietype',
                    id:'toolbar-lock-btn'
                },
                  {
                    name: getI18NString('删除'),
                    iconClass: 'q-icon  removetype',
                    id:'toolbar-remove-btn'
                },
            ]
            ,
            editor3: [
                ...
                // {
                //     name: getI18NString('粗细'),
                //     iconClass: 'q-icon toolbar-cx',
                //     id:'toolbar-edge-weight'
                // },
                // {
                //     name: getI18NString('箭头'),
                //     iconClass: 'q-icon toolbar-jt',
                //     id:'toolbar-edge-arrow'
                // },
                {
                    name: getI18NString('颜色'),
                    iconClass: 'q-icon toolbar-ys',
                    id:'toolbar-color-box'
                }

            ],
            editor7: [
                {
                    name: getI18NString('计算比对'),
                    iconClass: 'q-icon toolbar-calc',
                    id:'toolbar-calc-status'
                }

            ],
            search: {
                name: 'Find',
                placeholder: '输入关键字进行快速查找',
                ...
                search: function (name, info) {
                    ...
                    getGraph().forEach(function (e) {
                        if (e.name && reg.test(e.name) || e.idcard && reg.test(e.idcard) || e.tooltip && reg.test(e.tooltip)) {
                            result.push(e.id);
                        }
                    });
                    return result;
