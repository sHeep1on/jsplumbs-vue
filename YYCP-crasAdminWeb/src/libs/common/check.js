...
export const validateNameExsit = (rule, value, callback) => {
    var regEn = /[`~!@#$%^&*()_+<>?:"{},./;'[\]]/im
    var regCn = /[·！#￥（——）：；“”‘、，|《。》？、【】[\]]/im
    if (regEn.test(value) || regCn.test(value)) {
...
export const isNumber = (rule, value, callback) => {//纯数字校验
    ...
    if (value != '' && value != null && value != undefined) {
        if (!myreg.test(value)) {
            return callback(new Error('只能输入正整数数字'))
        }
    }
    return callback()
}
...
//限制输入框长度
export const remarkValid = (rule, value, callback) => {
    if (value && value.length > 8) {
      return callback(new Error("建议关系名称不要超过16个字符或8个汉字"));
    }
    return callback();
}
//  只能输入中文
export const isChina = (rule, value, callback) => {
  ...
  // 输入框中不能有空格
export const isHadBlank = (rule, value, callback) => {
  ...
  // IP地址
  export const ipAddress = (rule, value, callback) => {
    var myreg = new RegExp("^((2[0-4]\\d|25[0-5]|[1-9]?\\d|1\\d{2})\\.){3}(2[0-4]\\d|25[0-5]|[01]?\\d\\d?)$");
    if (!myreg.test(value)) {
  ...
  // 只能输入图片格式
  export const img = (rule, value, callback) => {
    var myreg = new RegExp("^(\.gif|\.jpeg|\.png|\.jpg|\.bmp)$");
    if (!myreg.test(value)) {
    return callback(new Error('只能输入图片格式'))
    }
    return callback()
}
